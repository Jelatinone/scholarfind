package com.github.jelatinone.infra.mock;
...
public class MockQueue<Value> implements RetryableQueue<Value> {
  ...
  public MockQueue(@NonNull QueueState drainedState) {
    this(drainedState, List.of());
  }

  public MockQueue(@NonNull QueueState drainedState, @NonNull Collection<? extends Value> initialMessages) {
  ...
  public synchronized MockQueue<Value> addInput(@NonNull Value message) {
    input.addLast(message);
    return this;
  }

  public synchronized MockQueue<Value> addInputs(@NonNull Collection<? extends Value> messages) {
  ...
  public synchronized QueueResult<Value> poll(int messageCount) {
    ...
    List<Envelope<Value>> messages = new ArrayList<>(limit);

    while (limit-- > 0 && !input.isEmpty()) {
      Value message = input.removeFirst();
      messages.add(new Envelope<>(message, acknowledgement(message)));
    }
  ...
  @Override
  public synchronized void send(@NonNull Value message) {
    output.addLast(message);
  }

  @Override
  public synchronized void sendRetry(@NonNull Value message) {
    retry.addLast(message);
  }

  @Override
  public synchronized void sendError(@NonNull Value message) {
