...
public final class SignalPlanner {
  ...
  public SignalPlannerResult classify(@NonNull AcquiredContent acquisition) {
    ...
    while (true) {
      ...
      if (stub.confidence() >= _configuration.dominanceConfiguration().minimumConfidence()) {
        return new SignalPlannerResult((stub), stub.confidence());
      }

      Candidate costlessCandidate = locateCandidate(currentAcquisition, stub, executed, true);
      if (costlessCandidate != null) {
        apply(costlessCandidate.extractor(), currentAcquisition, contributions);
        executed.add(costlessCandidate.extractor().identity());
        continue;
      }

      Candidate costfulCandidate = locateCandidate(currentAcquisition, stub, executed, false);
      if (costfulCandidate == null
          || costfulCandidate.utility() < _configuration.plannerConfiguration().minimumUtility()) {
        stub = buildStub(contributions);
        return new SignalPlannerResult(stub, stub.confidence());
      }

      if (!_configuration.plannerConfiguration().maxTier().includes(costfulCandidate.extractor().requires())) {
        stub = buildStub(contributions);
        return new SignalPlannerResult(stub, stub.confidence());
      }

      currentAcquisition = ensure(currentAcquisition, costfulCandidate.extractor().requires());
      apply(costfulCandidate.extractor(), currentAcquisition, contributions);
      executed.add(costfulCandidate.extractor().identity());
    }
  }
