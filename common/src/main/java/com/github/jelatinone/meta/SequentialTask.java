...
public non-sealed abstract class SequentialTask<Consumes, Produces>
					...
					case COLLECTING -> {
						...
						switch (result) {
							...
							case CollectionResult.Empty() -> {
								useMessage("Collection shape : Empty", INFO);
							}
						}
					...
					case POSTING -> {
						...
						switch (currentStatus) {
							...
							case PostResult.Fatal fatal -> {
								_attempts.remove(operand);
								useMessage(String.format("Failed work : %s", fatal.throwable()), ERROR);
							}
							...
							case PostResult.Retry retry -> {
								...
								if (attempt < _taskConfig.logicalRetries) {
									...
									_attempts.remove(operand);
									useMessage(String.format("Exhausted failed work : %s", retry.throwable()), ERROR);
								}
							}
						}
						useState(State.OPERATING);
	 ...
	 * 
	 */
	protected synchronized final void useState(final @NonNull State state) {
